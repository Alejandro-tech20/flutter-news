...
import 'package:noticias/negocio/noticias.dart';
import 'package:noticias/vista/Detalles.dart';
...
XmlElement find (RssItem item){
  var ti = xml.parse(item.toString());
  var tt = ti.findElements('media:content').first;
  return tt;
}
        ...
        child: Row(
          children: <Widget>[
                          ...
                          image: DecorationImage(
                            image: NetworkImage(
                              item.enclosure?.type == "image/jpeg"
                                  ? item.enclosure.url: item.content.url,//find(item),
                                  // : '',//ret(item),//---------------------------------------------
                            ),
                            fit: BoxFit.cover,
                          ),
            ...
            Container(
              ...
              child: Image.network(
                item.enclosure?.type == "image/jpeg" ? item.enclosure.url : item.content.url,
                fit: BoxFit.cover,
              ),
            ),
          ],
  ...
  // String ret(RssItem item) {
  //   var doc = RssFeed.parse(item.toString());
  //   //   // return findElementOrNull(
  //   //   //   parse(item.toString()), 'content')
  //   return '';
  //   // },
  // }
}
